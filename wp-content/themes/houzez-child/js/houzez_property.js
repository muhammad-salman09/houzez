...
jQuery(document).ready( function($) {
    ...
    if ( typeof houzezProperty !== "undefined" ) {
        ...
        var prop_lifestyle = houzezProperty.prop_lifestyle;
        var prop_region = houzezProperty.prop_region;
        ...
        /*  START CREATE LISTING FORM STEPS AND VALIDATION
         ...
         var add_new_property_validation = function() {
            ...
            $('.form-step.active select').change(function() {
                if ($(this).val() == '' || $(this).val() == null) {
                    $(this).next().find('button').addClass('form-control error');
                } else {
                    $(this).next().find('button').removeClass('form-control error');
                }
            });
                    ...
                    if(form.valid()){
                        ...
                        if (current == 1) {
                            if (prop_type == 1 && $('#prop_type').val() == '') {
                                $('#prop_type').next().find('button').addClass('form-control error');
                            }

                            if (prop_status == 1 && $('#prop_status').val() == '') {
                                $('#prop_status').next().find('button').addClass('form-control error');
                            }

                            if (prop_lifestyle == 1 && $('#prop_lifestyles').val() == null) {
                                $('#prop_lifestyles').next().find('button').addClass('form-control error');
                            }

                            if (prop_region == 1 && $('#prop_region').val() == '') {
                                $('#prop_region').next().find('button').addClass('form-control error');
                            }
            ...
            if(form.length > 0){ 
                form.validate({ // initialize plugin
                    ...
                    rules: {
                        ...
                        prop_lifestyle: {
                            required: houzez_validation(prop_lifestyle)
                        },
                        prop_region: {
                            required: houzez_validation(prop_region)
                        },
                        prop_labels: {
